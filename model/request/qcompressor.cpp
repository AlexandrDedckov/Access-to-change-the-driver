 ...
 */
bool QCompressor::gzipCompress(const QByteArray & input, QByteArray &output, int level)
{
    // Prepare output
    output.clear();
    ...
    // Initialize deflater
    int ret = deflateInit2(&strm, qMax(-1, qMin(9, level)), Z_DEFLATED, -8, 8, Z_DEFAULT_STRATEGY); //GZIP_WINDOWS_BIT
 ...
 */
bool QCompressor::gzipDecompress(const QByteArray & input, QByteArray &output, int type)
{
    // Prepare output
    output.clear();
    ...
    // Initialize inflater
    int ret = inflateInit2(&strm, type);//-8); //GZIP_WINDOWS_BIT
